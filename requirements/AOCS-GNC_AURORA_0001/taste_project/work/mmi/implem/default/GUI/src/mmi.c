...
void mmi_PI_Poll(void)
{
   ...
   while (!retrieve_message_from_queue(mmi_RI_queue_id,
      ...
      switch(message_received_type) {
         case i_TC:
            mmi_RI_TC((asn1SccT_Telecommand *)msgcontent);
            break;
       default : break;
      }
   }
...
void mmi_PI_TM
      (const asn1SccT_Telemetry *IN_telemetry)

{
   write_message_to_queue
      (mmi_PI_queue_id,
       sizeof(asn1SccT_Telemetry),
       (void*)IN_telemetry,
       i_TM);
