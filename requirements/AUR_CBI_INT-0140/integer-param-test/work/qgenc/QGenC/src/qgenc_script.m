...
if (exist('IntIface', 'file') == 4)
   load_system('IntIface');
   inportHan = find_system('IntIface','FindAll','on', 'SearchDepth', 1, 'BlockType','Inport');
   outportHan = find_system('IntIface','FindAll','on', 'SearchDepth', 1, 'BlockType','Outport');
...
add_block('simulink/Sources/In1', 'IntIface/input');
if all_inports_created
   set_param('IntIface/input','Position', inports_positions(inport_index,:));
else
   set_param('IntIface/input','Position',[25 (inport_index * 100) 55 ((inport_index * 100) + 14)]);
end
set_param('IntIface/input','BusOutputAsStruct','on');
set_param('IntIface/input','OutDataTypeStr','MyInteger');

outport_index = 0;
outport_index = outport_index + 1;
add_block('simulink/Sinks/Out1','IntIface/output');
if all_outports_created
   set_param('IntIface/output','Position', outports_positions(outport_index,:));
else
   set_param('IntIface/output','Position',[430 (outport_index * 100) 460 ((outport_index * 100) + 14)]);
end
set_param('IntIface/output','OutDataTypeStr','MyInteger');

set_param('IntIface','SaveOutput','off');
set_param('IntIface','SignalLogging','off');
set_param('IntIface','SaveTime','off')
set_param('IntIface','Solver','FixedStepDiscrete');
set_param('IntIface','SystemTargetFile','ert.tlc');
set_param('IntIface','TemplateMakefile','ert_default_tmf');
set_param('IntIface', 'PostCodeGenCommand', 'packNGo(buildInfo);');
set_param('IntIface','StrictBusMsg','ErrorLevel1')
save_system('IntIface');
close_system('IntIface');
