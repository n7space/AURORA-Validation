...
bool check_test_data(const Interface_Usage_Data data, const char *interface_name)
{
    ...
    if(data.cpu_usage_min_val <= 0.0 || data.cpu_usage_min_val >= 100.0)
    {
        print_rtems("Error: cpu_usage_min_val not in 0.0 - 100.0 range");
        return false;
    }

    if(data.cpu_usage_max_val <= 0.0 || data.cpu_usage_max_val >= 100.0)
    {
        print_rtems("Error: cpu_usage_max_val not in 0.0 - 100.0 range");
        return false;
    }

    if(data.cpu_usage_mean_val <= 0.0 || data.cpu_usage_mean_val >= 100.0)
    {
        print_rtems("Error: cpu_usage_mean_val not in 0.0 - 100.0 range");
        return false;
    }
